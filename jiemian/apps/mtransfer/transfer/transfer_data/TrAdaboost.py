...
class TradaBoostClassifier():
    ...
    def fit(self, trans_T, trans_S, label_T, label_S, test):
        ...
        weights_S = np.ones([self.row_S, 1]) / self.row_S
        weights_T = np.ones([self.row_T, 1]) / self.row_T
        weights = np.concatenate((weights_S, weights_T), axis=0)
        ...
        for i in range(self.epoch):
            ...
            # if error_rate < 0.001:
            #     self.beta_Tree[0, i] = np.max(self.beta_Tree) * 0.7
            #     self.epoch = i
            #     break
            if (i > 1) & (error_rate < 0.05):
                self.beta_Tree[0, i] = np.max(self.beta_Tree) * 0.7
                self.epoch = i
                break
